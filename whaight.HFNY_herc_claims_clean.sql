...
(	SELECT
	...
	WHERE
		...
		AND	(		CPT_Code_1			IN ( 'J9355' )
				...
				OR	(		ICD_Code_Type	=	'1'		--	Originally input as ICD 9 DX codes
				...
				OR	(		ICD_Code_Type	=	'2'		--	Originally input as ICD 10 DX codes
						AND	(		ICD_DX_Code_Admitting
...
/*	Query below is a test run for debugging big table load above	*/
SELECT DISTINCT
			crmc.claimNumber											::	VARCHAR( 13 )	AS	claim_ID
		,	CASE	crmc.ICD_10Indicator
				WHEN	'N'	THEN	'1'
				WHEN	'Y'	THEN	'2'
			END															::	VARCHAR(  1 )	AS	ICD_Code_Type
		,	REGEXP_REPLACE( crmc.AdmittingDXCode, '[^a-zA-Z0-9]+', '' )	::	VARCHAR(  8 )	AS	ICD_DX_Code_Admitting
		,	REGEXP_REPLACE( crmc.PrimaryDXCode, '[^a-zA-Z0-9]+', '' )	::	VARCHAR(  8 )	AS	ICD_DX_Code_Primary
